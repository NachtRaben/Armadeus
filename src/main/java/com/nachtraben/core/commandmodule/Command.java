...
public abstract class Command {
		...
		if (commandArgs.length >= 1) {
			...
			for (int i = 0; i < commandArgs.length; i++) {
				...
				if (!arg.isRequired)
					...
					if (arg.isDynamic) {
						if (arg.isRest) sb.append(restRegex); // Process {} tag
						else sb.append(requiredRegex); // Process <> tag
					} else {
						sb.append(arg.name);
					}
				}
	...
	public String toString() {
		...
		sb.append(this.getClass().getSimpleName()).append("(")
				...
				.append("aliases=").append(Arrays.toString(aliases)).append(", ")
				.append("flags=").append(Arrays.toString(flags))
				.append(")");
		return sb.toString();
	}

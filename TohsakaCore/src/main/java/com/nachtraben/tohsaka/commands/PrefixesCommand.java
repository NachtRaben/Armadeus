    ...
    public void run(CommandSender sender, Map<String, String> args, Map<String, String> flags) {
        if (sender instanceof GuildCommandSender) {
            ...
            if (config.getPrefixes() != null && !config.getPrefixes().isEmpty()) {
                ...
                sendee.sendMessage(ChannelTarget.GENERIC, "The prefixes are: `" + Tohsaka.getInstance().getConfig().getPrefixes() + "`");
            }
        }
        sender.sendMessage("The prefixes are: `" + Tohsaka.getInstance().getConfig().getPrefixes() + "`");
    }
}

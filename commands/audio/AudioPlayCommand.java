...
public class AudioPlayCommand extends Command {
    ...
    private static void trackLoaded(AudioTrack track, GuildCommandSender sender) {
    ...
    static void playlistLoaded(AudioPlaylist playlist, GuildCommandSender sender, boolean shuffle, int playlistLimit) {
        if (shuffle)
    ...
    public static void loadAndPlay(GuildCommandSender sender, String search, boolean shuffle, int playlistLimit) {
        boolean isSearch = search.startsWith("ytsearch:");
        if (isSearch) {
            sender.getGuildConfig().getMusicManager().getLink().getRestClient().getYoutubeSearchResult(search.replace("ytsearch:", "")).thenAccept(tracks -> {
                if (tracks.isEmpty()) {
            ...
            sender.getGuildConfig().getMusicManager().getLink().getRestClient().loadItem(search, new AudioLoadResultHandler() {
                ...
                public void trackLoaded(AudioTrack track) {
                    AudioPlayCommand.trackLoaded(track, sender);
                }

                @Override
                public void playlistLoaded(AudioPlaylist playlist) {
                    AudioPlayCommand.playlistLoaded(playlist, sender, shuffle, playlistLimit);
                }
